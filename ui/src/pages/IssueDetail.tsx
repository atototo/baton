...
import type { ActivityEvent } from "@atototo/shared";
import type { Agent, IssueAttachment } from "@atototo/shared";
...
const ACTION_LABEL_KEYS: Record<string, string> = {
  "issue.created": "issueDetail.actionCreated",
  "issue.updated": "issueDetail.actionUpdated",
  "issue.checked_out": "issueDetail.actionCheckedOut",
  "issue.released": "issueDetail.actionReleased",
  "issue.comment_added": "issueDetail.actionCommentAdded",
  "issue.attachment_added": "issueDetail.actionAttachmentAdded",
  "issue.attachment_removed": "issueDetail.actionAttachmentRemoved",
  "issue.deleted": "issueDetail.actionDeleted",
  "agent.created": "issueDetail.actionAgentCreated",
  "agent.updated": "issueDetail.actionAgentUpdated",
  "agent.paused": "issueDetail.actionAgentPaused",
  "agent.resumed": "issueDetail.actionAgentResumed",
  "agent.terminated": "issueDetail.actionAgentTerminated",
  "heartbeat.invoked": "issueDetail.actionHeartbeatInvoked",
  "heartbeat.cancelled": "issueDetail.actionHeartbeatCancelled",
  "approval.created": "issueDetail.actionApprovalCreated",
  "approval.approved": "issueDetail.actionApprovalApproved",
  "approval.rejected": "issueDetail.actionApprovalRejected",
};
...
function formatAction(action: string, details: Record<string, unknown> | null | undefined, t: (key: string, opts?: Record<string, unknown>) => string): string {
  if (action === "issue.updated" && details) {
    ...
    if (details.status !== undefined) {
      ...
      parts.push(
        from
          ? t("issueDetail.changedStatusFromTo", { from: humanizeValue(from), to: humanizeValue(details.status) })
          : t("issueDetail.changedStatusTo", { to: humanizeValue(details.status) })
      );
    }
    if (details.priority !== undefined) {
      ...
      parts.push(
        from
          ? t("issueDetail.changedPriorityFromTo", { from: humanizeValue(from), to: humanizeValue(details.priority) })
          : t("issueDetail.changedPriorityTo", { to: humanizeValue(details.priority) })
      );
    }
    if (details.assigneeAgentId !== undefined || details.assigneeUserId !== undefined) {
      parts.push(
        details.assigneeAgentId || details.assigneeUserId
          ? t("issueDetail.assignedIssue")
          : t("issueDetail.unassignedIssue"),
      );
    }
    if (details.title !== undefined) parts.push(t("issueDetail.updatedTitle"));
    if (details.description !== undefined) parts.push(t("issueDetail.updatedDescription"));
...
export function IssueDetail() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: issue?.title ?? issueId ?? t("nav.issues") },
    ]);
  }, [setBreadcrumbs, issue, issueId, t]);
      ...
      <Tabs value={detailTab} onValueChange={setDetailTab} className="space-y-3">
        ...
        <TabsContent value="comments">
          <CommentThread
            ...
            agentMap={agentMap}
            draftKey={`baton:issue-comment-draft:${issue.id}`}
            enableReassign
        ...
        >
          <CollapsibleTrigger className="flex w-full items-center justify-between px-3 py-2 text-left">
            <span className="text-sm font-medium text-muted-foreground">
              {t("issueDetail.linkedApprovals", { count: linkedApprovals.length })}
            </span>
            <ChevronDown
              ...
              {!issueCostSummary.hasCost && !issueCostSummary.hasTokens ? (
                ...
                <div className="flex flex-wrap gap-3 text-xs text-muted-foreground">
                  ...
                  {issueCostSummary.hasTokens && (
                    <span>
                      {t("issueDetail.tokens")} {formatTokens(issueCostSummary.totalTokens)}
