...
type BatonSprite = (typeof BATON_SPRITES)[number];
...
function spriteSize(sprite: BatonSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = BATON_SPRITES[(baseCol + baseRow) % BATON_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = BATON_SPRITES[Math.floor(Math.random() * BATON_SPRITES.length)]!;
