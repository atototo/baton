...
// Canonical type lives in @atototo/adapter-utils; re-exported here
// so existing imports from this file keep working.
export type { CreateConfigValues } from "@atototo/adapter-utils";
import type { CreateConfigValues } from "@atototo/adapter-utils";
...
function buildCreateValuesForAdapterSwitch(
...
function buildPreservedAdapterConfigForSwitch(
  adapterType: string,
  current: Record<string, unknown>,
): Record<string, unknown> {
  const nextConfig: Record<string, unknown> = {
    cwd: current.cwd,
    instructionsFilePath: current.instructionsFilePath,
    promptTemplate: current.promptTemplate,
    bootstrapPromptTemplate: current.bootstrapPromptTemplate,
    model:
...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const createSecret = useMutation({
    mutationFn: (input: { name: string; value: string }) => {
      if (!selectedCompanyId)
        throw new Error(t("agentConfig.errors.selectCompanyCreateSecrets"));
  ...
  const uploadMarkdownImage = useMutation({
    mutationFn: async ({
      ...
      if (!selectedCompanyId)
        throw new Error(t("agentConfig.errors.selectCompanyUploadImages"));
  ...
  const testEnvironment = useMutation({
    mutationFn: async () => {
      if (!selectedCompanyId) {
        throw new Error(t("agentConfig.errors.selectCompanyTestEnvironment"));
      }
  ...
  const codexSearchEnabled =
    adapterType === "codex_local"
      ...
      : false;
  const currentAdapterLabel = adapterLabels[adapterType] ?? adapterType;
  const adapterHelpTypeKey = `inlineHelp.adapter.types.${adapterType}` as const;
  ...
  return (
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      >
        ...
        >
          <InlineHelp
            title={t("inlineHelp.adapter.title", {
              adapter: currentAdapterLabel,
            })}
            summary={t("inlineHelp.adapter.summary")}
          >
            <ul className="space-y-1.5">
              <li>{t(`${adapterHelpTypeKey}.description`)}</li>
              <li>{t(`${adapterHelpTypeKey}.recommendation`)}</li>
              <li>
                {isLocal
                  ? t("inlineHelp.adapter.localNote")
                  : t("inlineHelp.adapter.remoteNote")}
              </li>
            </ul>
          </InlineHelp>
          <Field label={t("agentConfig.adapterType")} hint={help.adapterType}>
            <AdapterTypeDropdown
              ...
              onChange={(t) => {
                if (isCreate) {
                  set!(buildCreateValuesForAdapterSwitch(props.values, t));
                } else {
                  const currentEffectiveConfig = {
                    ...config,
                  ...
                  setOverlay((prev) => ({
                    ...prev,
                    adapterType: t,
                    adapterConfig: buildPreservedAdapterConfigForSwitch(
                      t,
          ...
          {testEnvironment.error && (
            <div className="rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-destructive">
              {testEnvironment.error instanceof Error
                ...
                : t("agentConfig.errors.environmentTestFailed")}
            </div>
          )}
        ...
        <div className={cn(!cards && "border-b border-border")}>
          {cards ? (
            <h3 className="text-sm font-medium mb-3">
              {t("agentConfig.permissionsAndConfiguration")}
            </h3>
          ) : (
            <div className="px-4 py-2 text-xs font-medium text-muted-foreground">
              {t("agentConfig.permissionsAndConfiguration")}
          ...
          >
            ...
            <Field
              label={t("agentConfig.bootstrapPrompt")}
            ...
            {!isCreate && (
              <>
                ...
                <Field
                  label={t("agentConfig.interruptGracePeriodSec")}
                  hint={help.graceSec}
                >
                  <DraftNumberInput
                    value={eff(
        ...
        <div className={cn(!cards && "border-b border-border")}>
          {cards ? (
            <h3 className="text-sm font-medium flex items-center gap-2 mb-3">
              <Heart className="h-3 w-3" /> {t("agentConfig.runPolicy")}
            </h3>
          ) : (
          ...
          >
            ...
            >
              <div className="space-y-3">
                ...
                <Field
                  label={t("agentConfig.maxConcurrentRuns")}
          ...
          >
            ...
            {check.hint && (
              <span className="block opacity-90 break-words">
                {t("agentConfig.hintPrefix")} {check.hint}
              </span>
            )}
          </div>
        ))}
...
function EnvVarEditor({
  ...
  async function sealRow(i: number) {
    ...
    try {
      ...
      setSealError(
        err instanceof Error ? err.message : t("agentConfig.errors.failedToCreateSecret")
      );
    }
  }
        ...
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected ? selected.label : value || t("agentConfig.defaultModel")}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
    ...
    <Field label={t("agentConfig.thinkingEffort")} hint={help.thinkingEffort}>
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected?.label ?? t("agentConfig.auto")}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
