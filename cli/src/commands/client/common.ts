...
import { ApiRequestError, BatonApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Baton config file")
    .option("-d, --data-dir <path>", "Baton data directory root (isolates state from ~/.baton)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Baton API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.BATON_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const apiKey =
    options.apiKey?.trim() ||
    process.env.BATON_API_KEY?.trim() ||
    readKeyFromProfileEnv(profile);

  const companyId =
    options.companyId?.trim() ||
    process.env.BATON_COMPANY_ID?.trim() ||
    profile.companyId;
  ...
  const api = new BatonApiClient({ apiBase, apiKey });
  return {
    api,
    companyId,
    profileName,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.BATON_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.BATON_SERVER_PORT || "");
