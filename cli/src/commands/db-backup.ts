...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@atototo/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://baton:baton@127.0.0.1:${port}/baton`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  printBatonCliBanner();
  p.intro(pc.bgCyan(pc.black(" baton db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveBatonInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "baton";
