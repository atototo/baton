...
function defaultConfig(): BatonConfig {
...
export async function configure(opts: {
  ...
  printBatonCliBanner();
  p.intro(pc.bgCyan(pc.black(" baton configure ")));
  const configPath = resolveConfigPath(opts.config);

  if (!configExists(opts.config)) {
    p.log.error("No config file found. Run `atototo onboard` first.");
    p.outro("");
    return;
  }
      ...
      case "secrets":
        config.secrets = await promptSecrets(config.secrets);
        {
          ...
          if (keyResult.status === "created") {
            ...
            p.log.message(pc.dim("Skipping local key file management because BATON_SECRETS_MASTER_KEY is set"));
          }
        }
        break;
    }
