...
const DEFAULT_AGENT_JWT_ISSUER = "baton";
const DEFAULT_AGENT_JWT_AUDIENCE = "baton-api";
...
function defaultSecretsKeyFilePath(): string {
  return resolveDefaultSecretsKeyFilePath(resolveBatonInstanceId());
}
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveBatonInstanceId());
}

export async function envCommand(opts: { config?: string }): Promise<void> {
  p.intro(pc.bgCyan(pc.black(" baton env ")));

  const configPath = resolveConfigPath(opts.config);
  let config: BatonConfig | null = null;
...
function collectDeploymentEnvRows(config: BatonConfig | null, configPath: string): EnvVarRow[] {
  ...
  const rows: EnvVarRow[] = [
    ...
    {
      key: "BATON_AGENT_JWT_TTL_SECONDS",
      value: process.env.BATON_AGENT_JWT_TTL_SECONDS ?? DEFAULT_AGENT_JWT_TTL_SECONDS,
      source: process.env.BATON_AGENT_JWT_TTL_SECONDS ? "env" : "default",
      required: false,
      note: "JWT lifetime in seconds",
    },
    {
      key: "BATON_AGENT_JWT_ISSUER",
      value: process.env.BATON_AGENT_JWT_ISSUER ?? DEFAULT_AGENT_JWT_ISSUER,
      source: process.env.BATON_AGENT_JWT_ISSUER ? "env" : "default",
      required: false,
      note: "JWT issuer",
    },
    {
      key: "BATON_AGENT_JWT_AUDIENCE",
      value: process.env.BATON_AGENT_JWT_AUDIENCE ?? DEFAULT_AGENT_JWT_AUDIENCE,
      source: process.env.BATON_AGENT_JWT_AUDIENCE ? "env" : "default",
      required: false,
      note: "JWT audience",
    },
    {
    ...
    {
      key: "BATON_SECRETS_PROVIDER",
      value: secretsProvider,
      source: process.env.BATON_SECRETS_PROVIDER
        ? "env"
        : config?.secrets?.provider
          ? "config"
          : "default",
    ...
    {
      key: "BATON_SECRETS_STRICT_MODE",
      value: secretsStrictMode,
      source: process.env.BATON_SECRETS_STRICT_MODE
        ? "env"
    ...
    {
      key: "BATON_SECRETS_MASTER_KEY_FILE",
      value: secretsKeyFilePath,
      source: process.env.BATON_SECRETS_MASTER_KEY_FILE
        ? "env"
        : config?.secrets?.localEncrypted?.keyFilePath
          ? "config"
          : "default",
    ...
    {
      key: "BATON_STORAGE_PROVIDER",
      value: storageProvider,
      source: process.env.BATON_STORAGE_PROVIDER
        ? "env"
        : config?.storage?.provider
          ? "config"
          : "default",
    ...
    {
      key: "BATON_STORAGE_LOCAL_DIR",
      value: storageLocalDir,
      source: process.env.BATON_STORAGE_LOCAL_DIR
        ? "env"
        : config?.storage?.localDisk?.baseDir
          ? "config"
          : "default",
    ...
    {
      key: "BATON_STORAGE_S3_BUCKET",
      value: storageS3Bucket,
      source: process.env.BATON_STORAGE_S3_BUCKET
        ? "env"
        : config?.storage?.s3?.bucket
          ? "config"
          : "default",
    ...
    {
      key: "BATON_STORAGE_S3_REGION",
      value: storageS3Region,
      source: process.env.BATON_STORAGE_S3_REGION
        ? "env"
        : config?.storage?.s3?.region
          ? "config"
          : "default",
    ...
    {
      key: "BATON_STORAGE_S3_ENDPOINT",
      value: storageS3Endpoint,
      source: process.env.BATON_STORAGE_S3_ENDPOINT
        ? "env"
        : config?.storage?.s3?.endpoint
          ? "config"
          : "default",
    ...
    {
      key: "BATON_STORAGE_S3_PREFIX",
      value: storageS3Prefix,
      source: process.env.BATON_STORAGE_S3_PREFIX
        ? "env"
        : config?.storage?.s3?.prefix
          ? "config"
          : "default",
    ...
    {
      key: "BATON_STORAGE_S3_FORCE_PATH_STYLE",
      value: storageS3ForcePathStyle,
      source: process.env.BATON_STORAGE_S3_FORCE_PATH_STYLE
        ? "env"
  ...
  if (process.env.BATON_CONFIG || configPath !== defaultConfigPath) {
    rows.push({
      key: "BATON_CONFIG",
      value: process.env.BATON_CONFIG ?? configPath,
      source: process.env.BATON_CONFIG ? "env" : "default",
      required: false,
