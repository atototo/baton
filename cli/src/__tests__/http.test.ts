...
describe("BatonApiClient", () => {
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new BatonApiClient({
    ...
    expect(headers["x-baton-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new BatonApiClient({ apiBase: "http://localhost:3100" });
