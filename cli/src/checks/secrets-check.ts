...
export function secretsCheck(config: BatonConfig, configPath?: string): CheckResult {
  ...
  const envMasterKey = process.env.BATON_SECRETS_MASTER_KEY;
  if (envMasterKey && envMasterKey.trim().length > 0) {
    if (!decodeMasterKey(envMasterKey)) {
      return {
        ...
        status: "fail",
        message:
          "BATON_SECRETS_MASTER_KEY is invalid (expected 32-byte base64, 64-char hex, or raw 32-char string)",
        canRepair: false,
        repairHint: "Set BATON_SECRETS_MASTER_KEY to a valid key or unset it to use a key file",
      };
    }
    ...
    return withStrictModeNote(
      {
        name: "Secrets adapter",
        status: "pass",
        message: "Local encrypted provider configured via BATON_SECRETS_MASTER_KEY",
      },
      config,
    );
  }

  const keyFileOverride = process.env.BATON_SECRETS_MASTER_KEY_FILE;
  ...
  try {
    ...
    return {
      ...
      canRepair: false,
      repairHint: "Check file permissions or set BATON_SECRETS_MASTER_KEY",
    };
  }
