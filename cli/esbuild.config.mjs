/**
 * esbuild configuration for building the atototo CLI for npm.
 *
 * Bundles all workspace packages (@atototo/*) into a single file.
...
// to npm and resolved at runtime (e.g. @atototo/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@atototo/")) {
      externals.add(name);
    }
  }
