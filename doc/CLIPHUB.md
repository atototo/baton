...
ClipHub is the public registry where people share, discover, and download Baton company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Baton what a package registry is to a programming language. Baton already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Baton — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Baton instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Baton spec. It contains:
...
**Install (fresh start):**
```
baton install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Baton instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Baton, export your company as a template, then publish:

```
baton export --template my-company
baton publish cliphub my-company
```
...
ClipHub is a **separate service** from Baton itself. Baton is self-hosted; ClipHub is a hosted registry that Baton instances talk to.
...
| **Baton CLI** | `atototo install`, `atototo publish`, `atototo cliphub sync` — built into Baton |
| **Baton UI** | "Browse ClipHub" panel in the Baton web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Baton) |
| Backend | TypeScript + Hono (consistent with Baton) |
| Database | PostgreSQL |
...
4. Run `atototo install cliphub:acme/lean-saas-shop`
5. Baton creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Baton until it works well
2. Export: `atototo export --template my-agency`
3. Publish: `atototo publish cliphub my-agency`
...
3. Install just that agent: `atototo install cliphub:acme/senior-python-eng --agent`
...
ClipHub is **not required** to use Baton. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Baton what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `atototo install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Baton web UI
- [ ] `atototo cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Baton's own philosophy)
