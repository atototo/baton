...
Baton's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Baton's API, UI, and data model | Yes |
...
modules/
  observability/
    baton.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@atototo/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@atototo/core";
...
```
1. Scan modules/ directory for baton.module.json manifests
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@atototo/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@atototo/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:baton/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.baton.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm atototo store list                    # browse available modules and templates
pnpm atototo store install <module-id>     # install a module
pnpm atototo store import <template-id>    # import a company template
pnpm atototo store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Baton instance | `server:started` |
...
4. **Module config** — `baton.config.json` with per-module config, env var interpolation
...
10. **`@atototo/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
