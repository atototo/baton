...
Add a single storage subsystem for Baton that supports:
...
- API serves attachment bytes through authenticated Baton endpoints.
...
- Default local path is under instance root: `~/.baton/instances/<instanceId>/data/storage`.
...
- `atototo onboard` writes a valid `storage` config block by default.
- `atototo configure --section storage` can switch between local and s3 modes.
...
- `atototo doctor` reports actionable storage status.
