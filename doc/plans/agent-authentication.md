...
Agents need API keys to authenticate with Baton. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Baton
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Baton generates a token and passes it directly to the agent
...
`BATON_API_URL`, `BATON_AGENT_ID`, `BATON_COMPANY_ID`. We need to
add a `BATON_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Baton CLI
...
```
baton auth login
# Opens browser -> user confirms -> token stored at ~/.baton/credentials
```
...
managed by the Baton server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Baton.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Baton SKILL.md (or a link to it)
   - What information Baton needs from the agent (e.g. webhook URL, adapter
...
5. Baton stores the pending registration.
...
7. On approval, Baton provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Baton issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.baton.ing/skills/baton/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Baton should send you heartbeats",
...
1. Generate a Baton invite link for the company.
...
4. A Baton company member approves the new agent.
5. Baton begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `BATON_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `atototo auth login` for developer-managed remote agents.                                      |
