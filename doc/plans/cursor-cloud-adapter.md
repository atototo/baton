...
This document defines the V1 design for a Baton adapter that integrates with
...
4. synthesized stdout events for Baton UI/CLI
...
3. **Agent callback auth to Baton** uses a bootstrap exchange flow (no long-lived Baton key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Baton endpoints, not full SKILL.md prompt injection.
...
- `batonPublicUrl` (optional override; else `BATON_PUBLIC_URL` env)
...
## Baton Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `BATON_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.batonPublicUrl`
2. `process.env.BATON_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Baton credentials in prompt text.

Flow:

1. Before launch/follow-up, Baton mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Baton validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Baton API calls.
...
1. Prompt includes a compact instruction to fetch skills from Baton.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/baton`
   - `GET /api/skills/baton-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <batonPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Baton run context.
...
5. `batonPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Baton skill endpoints |
| Baton auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Baton context.
3. Add richer PR metadata surfacing in Baton UI.
