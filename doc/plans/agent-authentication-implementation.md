...
- New config/env settings:
  - `BATON_AGENT_JWT_SECRET`
  - `BATON_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `BATON_AGENT_JWT_ISSUER` (default: `baton`)
  - `BATON_AGENT_JWT_AUDIENCE` (default: `baton-api`)
...
1. In:
   ...
   inject `BATON_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `BATON_API_KEY`, do not overwrite it.
- Continue injecting:
  - `BATON_AGENT_ID`
  - `BATON_COMPANY_ID`
  - `BATON_API_URL`
...
- Local adapters authenticate without manual `BATON_API_KEY` config.
