...
This spec defines how Baton actually runs agents while staying runtime-agnostic.
...
1. Baton is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Baton manages lifecycle, and on exit Baton parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `baton.skill` (shared Baton skill text block)
...
2. Preferred transport is env vars (`BATON_*`) injected at runtime.
