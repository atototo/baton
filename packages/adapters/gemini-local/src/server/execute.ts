...
function renderBatonEnvNote(env: Record<string, string>): string {
  const batonKeys = Object.keys(env)
    .filter((key) => key.startsWith("BATON_"))
    .sort();
  if (batonKeys.length === 0) return "";
  return [
    "Baton runtime note:",
    `The following BATON_* environment variables are available in this run: ${batonKeys.join(", ")}`,
...
function renderApiAccessNote(env: Record<string, string>): string {
  if (!hasNonEmptyEnvValue(env, "BATON_API_URL") || !hasNonEmptyEnvValue(env, "BATON_API_KEY")) return "";
  return [
    "Baton API access note:",
    "Use run_shell_command with curl to make Baton API requests.",
    "GET example:",
    `  run_shell_command({ command: "curl -s -H \\"Authorization: Bearer $BATON_API_KEY\\" \\"$BATON_API_URL/api/agents/me\\"" })`,
    "POST/PATCH example:",
    `  run_shell_command({ command: "curl -s -X POST -H \\"Authorization: Bearer $BATON_API_KEY\\" -H 'Content-Type: application/json' -H \\"X-Baton-Run-Id: $BATON_RUN_ID\\" -d '{...}' \\"$BATON_API_URL/api/issues/{id}/checkout\\"" })`,
    "",
    "",
  ].join("\n");
}

// TODO: Port skill injection utilities from Paperclip's adapter-utils
// (ensureBatonSkillSymlink, readBatonRuntimeSkillEntries, resolveBatonDesiredSkillNames, removeMaintainerOnlySkillSymlinks)
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // TODO: Re-enable skill injection once skill utilities are ported
  // const geminiSkillEntries = await readBatonRuntimeSkillEntries(config, __moduleDir);
  // const desiredGeminiSkillNames = resolveBatonDesiredSkillNames(config, geminiSkillEntries);
  ...
  const linkedIssueIds = Array.isArray(context.issueIds)
    ...
    : [];
  if (wakeTaskId) env.BATON_TASK_ID = wakeTaskId;
  if (wakeReason) env.BATON_WAKE_REASON = wakeReason;
  if (wakeCommentId) env.BATON_WAKE_COMMENT_ID = wakeCommentId;
  if (approvalId) env.BATON_APPROVAL_ID = approvalId;
  if (approvalStatus) env.BATON_APPROVAL_STATUS = approvalStatus;
  if (linkedIssueIds.length > 0) env.BATON_LINKED_ISSUE_IDS = linkedIssueIds.join(",");
  if (effectiveWorkspaceCwd) env.BATON_WORKSPACE_CWD = effectiveWorkspaceCwd;
  if (workspaceSource) env.BATON_WORKSPACE_SOURCE = workspaceSource;
  if (workspaceId) env.BATON_WORKSPACE_ID = workspaceId;
  if (workspaceRepoUrl) env.BATON_WORKSPACE_REPO_URL = workspaceRepoUrl;
  if (workspaceRepoRef) env.BATON_WORKSPACE_REPO_REF = workspaceRepoRef;
  if (agentHome) env.AGENT_HOME = agentHome;
  if (workspaceHints.length > 0) env.BATON_WORKSPACES_JSON = JSON.stringify(workspaceHints);
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[baton] Gemini session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[baton] Gemini resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
