...
// and re-enable ensurePiSkillsInjected() below.

const BATON_SESSIONS_DIR = path.join(os.homedir(), ".pi", "batons");
...
async function ensureSessionsDir(): Promise<string> {
  await fs.mkdir(BATON_SESSIONS_DIR, { recursive: true });
  return BATON_SESSIONS_DIR;
}

function buildSessionPath(agentId: string, timestamp: string): string {
  const safeTimestamp = timestamp.replace(/[:.]/g, "-");
  return path.join(BATON_SESSIONS_DIR, `${safeTimestamp}-${agentId}.jsonl`);
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  // const piSkillEntries = await readBatonRuntimeSkillEntries(config, __moduleDir);
  // const desiredPiSkillNames = resolveBatonDesiredSkillNames(config, piSkillEntries);
  ...
  if (workspaceCwd) env.BATON_WORKSPACE_CWD = workspaceCwd;
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[baton] Pi session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (resolvedInstructionsFilePath) {
    try {
      ...
      await onLog(
        "stdout",
        `[baton] Warning: could not read agent instructions file "${resolvedInstructionsFilePath}": ${reason}\n`,
      );
  ...
  const composedSuffix = composedInstructions ? `\n\n---\n\n${composedInstructions}` : "";
  const renderedSystemPromptExtension = renderTemplate(systemPromptExtension, templateData) + composedSuffix;
  ...
  const buildArgs = (sessionFile: string): string[] => {
    ...
    // TODO: Re-enable once skill injection is ported
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[baton] Pi session "${runtimeSessionId}" is unavailable; retrying with a fresh session.\n`,
    );
