...
import type { AdapterModel } from "@atototo/adapter-utils";
import { asString, runChildProcess } from "@atototo/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.BATON_PI_COMMAND === "string" &&
    process.env.BATON_PI_COMMAND.trim().length > 0
      ? process.env.BATON_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["BATON_", "npm_", "NPM_"] as const;
