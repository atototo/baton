...
Use when:
- You want Baton to run Pi (the AI coding agent) locally as the agent runtime
...
- Baton requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/batons/ and resumed with --session.
