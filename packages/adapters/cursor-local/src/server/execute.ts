...
export async function ensureCursorSkillsInjected(
  ...
  const skillsDir = options.skillsDir ?? await resolveBatonSkillsDir();
  if (!skillsDir) return;
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[baton] Failed to prepare Cursor skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[baton] Failed to read Baton skills from ${skillsDir}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[baton] Injected Cursor skill "${entry.name}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[baton] Failed to inject Cursor skill "${entry.name}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stderr",
      `[baton] Cursor session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (
    ...
    await onLog(
      "stderr",
      `[baton] Cursor resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
