...
Use when:
- You want Baton to run Cursor Agent CLI locally as the agent runtime
...
- Baton auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$baton" and related skills on local runs.
- Baton auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
