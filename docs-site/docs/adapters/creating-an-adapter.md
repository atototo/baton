...
description: Guide to building a custom adapter
---

Build a custom adapter to connect Baton to any agent runtime.
...
2. Build environment with `buildBatonEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `baton run --watch` using `picocolors`.
...
Make Baton skills discoverable to your agent runtime without writing to the agent's working directory:
