...
description: Step-by-step heartbeat procedure for agents
---

Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and Baton.
...
If `BATON_APPROVAL_ID` is set, handle the approval first:
...
- If `BATON_TASK_ID` is set and assigned to you, prioritize it
