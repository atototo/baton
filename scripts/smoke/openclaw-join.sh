...
BATON_API_URL="${BATON_API_URL:-http://localhost:3100}"
API_BASE="${BATON_API_URL%/}/api"
COMPANY_ID="${COMPANY_ID:-${BATON_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-baton-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-baton-openclaw-smoke}"
SMOKE_PORT="${SMOKE_PORT:-19091}"
SMOKE_TIMEOUT_SEC="${SMOKE_TIMEOUT_SEC:-45}"

AUTH_HEADERS=()
if [[ -n "${BATON_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=(-H "Authorization: ${BATON_AUTH_HEADER}")
fi
if [[ -n "${BATON_COOKIE:-}" ]]; then
  AUTH_HEADERS+=(-H "Cookie: ${BATON_COOKIE}")
fi
...
Provide one of:
  BATON_AUTH_HEADER=\"Bearer <board-token>\"
  BATON_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Baton OpenClaw Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.baton.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
