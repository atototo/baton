...
Baton supports two runtime modes with different security profiles.
...
```sh
pnpm atototo allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Baton emits a one-time claim URL at startup:
...
```sh
BATON_DEPLOYMENT_MODE=authenticated pnpm atototo run
```
