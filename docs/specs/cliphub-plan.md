# ClipHub: Marketplace for Baton Team Configurations

> The "app store" for whole-company AI teams — pre-built Baton configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Baton-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Baton company (agents, projects, governance) |
...
A complete Baton company configuration:
...
Single-agent configurations designed to plug into a Baton org:
...
Portable skill files that any Baton agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Baton's skill loading system
...
interface Listing {
  ...
  batonVersionMin: string;     // Minimum Baton version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Baton company
...
| `POST` | `/api/listings/:id/install` | Install to Baton company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Baton company (or create new)
  → ClipHub API calls Baton API to:
  ...
  → Redirect to Baton dashboard with new team running
```
...
### 5.3 Creator: Export from Baton → Publish

```
Running Baton company → "Export as Blueprint" (CLI or UI)
  → Baton exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Baton brand blue/purple)
...
| Install flow | One-click deploy to Baton company |
...
{
  "targetCompanyId": "uuid",         // Existing Baton company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `baton-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Baton
- **Backend**: Node.js API (or extend Baton server)
- **Database**: Postgres (can share Baton's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Baton auth (or OAuth2)

### 9.2 Integration with Baton

ClipHub can be:
- **Option A**: A separate app that calls Baton's API to install blueprints
- **Option B**: A built-in section of the Baton UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Baton UI and API.
...
- [ ] Install flow: blueprint → Baton company
