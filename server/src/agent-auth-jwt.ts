...
function jwtConfig() {
  const secret = process.env.BATON_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.BATON_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.BATON_AGENT_JWT_ISSUER ?? "baton",
    audience: process.env.BATON_AGENT_JWT_AUDIENCE ?? "baton-api",
  };
}
