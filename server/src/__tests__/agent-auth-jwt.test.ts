...
describe("agent local JWT", () => {
  const secretEnv = "BATON_AGENT_JWT_SECRET";
  const ttlEnv = "BATON_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "BATON_AGENT_JWT_ISSUER";
  const audienceEnv = "BATON_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "baton";
    process.env[audienceEnv] = "baton-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
