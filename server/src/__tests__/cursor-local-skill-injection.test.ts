...
import { ensureCursorSkillsInjected } from "@atototo/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Baton skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("baton-cursor-skills-src-");
    const skillsHome = await makeTempDir("baton-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "baton");
    await createSkillDir(skillsDir, "baton-create-agent");
    ...
    const injectedA = path.join(skillsHome, "baton");
    const injectedB = path.join(skillsHome, "baton-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "baton")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "baton-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "baton"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "baton-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("baton-cursor-preserve-src-");
    const skillsHome = await makeTempDir("baton-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "baton");
    ...
    expect((await fs.lstat(path.join(skillsHome, "baton-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("baton-cursor-fail-src-");
    const skillsHome = await makeTempDir("baton-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
