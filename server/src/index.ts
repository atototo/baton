...
if (process.env.BATON_SECRETS_PROVIDER === undefined) {
  process.env.BATON_SECRETS_PROVIDER = config.secretsProvider;
}
if (process.env.BATON_SECRETS_STRICT_MODE === undefined) {
  process.env.BATON_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
}
if (process.env.BATON_SECRETS_MASTER_KEY_FILE === undefined) {
  process.env.BATON_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
}
...
async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
  if (process.env.BATON_MIGRATION_PROMPT === "never") return false;
  if (process.env.BATON_MIGRATION_AUTO_APPLY === "true") return true;
...
const LOCAL_BOARD_USER_EMAIL = "local@baton.local";
...
if (config.databaseUrl) {
  ...
  const verboseEmbeddedPostgresLogs = process.env.BATON_EMBEDDED_POSTGRES_VERBOSE === "true";
  ...
  const embeddedAdminConnectionString = `postgres://baton:baton@127.0.0.1:${port}/postgres`;
  const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "baton");
  if (dbStatus === "created") {
    logger.info("Created embedded PostgreSQL database: baton");
  }

  const embeddedConnectionString = `postgres://baton:baton@127.0.0.1:${port}/baton`;
...
if (config.deploymentMode === "authenticated") {
  ...
  const betterAuthSecret =
    process.env.BETTER_AUTH_SECRET?.trim() ?? process.env.BATON_AGENT_JWT_SECRET?.trim();
  if (!betterAuthSecret) {
    throw new Error(
      "authenticated mode requires BETTER_AUTH_SECRET (or BATON_AGENT_JWT_SECRET) to be set",
    );
  }
...
const runtimeApiHost =
  runtimeListenHost === "0.0.0.0" || runtimeListenHost === "::"
    ? "localhost"
    : runtimeListenHost;
process.env.BATON_LISTEN_HOST = runtimeListenHost;
process.env.BATON_LISTEN_PORT = String(listenPort);
process.env.BATON_API_URL = `http://${runtimeApiHost}:${listenPort}`;
...
server.listen(listenPort, config.host, () => {
  ...
  if (process.env.BATON_OPEN_ON_LISTEN === "true") {
