...
import { createIssueAttachmentMetadataSchema } from "@atototo/shared";
...
export function issueAttachmentRoutes(db: Db, storage: StorageService) {
  const router = Router();
  ...
  router.param("id", async (req, _res, next, rawId) => {
    try {
      if (/^[A-Z]+-\d+$/i.test(rawId)) {
        ...
        if (issue) req.params.id = issue.id;
      }
      next();
    } catch (err) {
      next(err);
  ...
  // Resolve issueId param for company-scoped attachment routes.
